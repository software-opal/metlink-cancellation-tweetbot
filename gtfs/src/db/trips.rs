...
use crate::{
    gtfs::data::{stop::PickupDropoffType, time::Time},
};
...
impl From<&crate::gtfs::data::GtfsData> for TripDb {
    fn from(parsed: &crate::gtfs::data::GtfsData) -> Self {
        let mut trip_stops: BTreeMap<String, Vec<TripStop>> = BTreeMap::new();
        parsed.stop_time.iter().for_each(|s| {
            if let Some(v) = trip_stops.get_mut(&s.trip_id) {
                v.push(s.into())
            } else {
                trip_stops.insert(s.trip_id.clone(), vec![s.into()]);
            }
        });
        ...
        Self {
            trips: parsed
                ...
                .map(|trip| {
                    ...
                    trip.stops = trip_stops.remove(&trip.id).unwrap();
                    trip
                })
