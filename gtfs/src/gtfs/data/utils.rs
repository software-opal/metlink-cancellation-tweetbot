...
pub fn deserialize_num_bool<'de, D>(deserializer: D) -> Result<bool, D::Error> where D: Deserializer<'de> {
    let s: u8 = Deserialize::deserialize(deserializer)?;
    match s {
        0 => Ok(false),
        1 => Ok(true),
        _ => Err(serde::de::Error::custom(format!("Invalid numeric boolean value: {}", s)))
    }
}
